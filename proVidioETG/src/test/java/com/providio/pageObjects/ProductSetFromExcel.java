		...
		try {
			...
			while (true) {
				...
				if (randomValue != null) {
					...
					try {
						List<WebElement> elementList = driver.findElements(By.xpath("//input[@name='q']"));
						Thread.sleep(2000);
					...
					Thread.sleep(2000);
					List<WebElement> searchedForProductList= driver.findElements(By.xpath("(//span[contains(@class,'product-id')])[2]"));
					if(searchedForProductList.size()>0) {					
